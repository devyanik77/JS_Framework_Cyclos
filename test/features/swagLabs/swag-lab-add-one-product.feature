...
Feature: Swag Labs Add One Product Functionality
    ...
    @TC005
    Scenario Outline: dd one product to cart page
    ...
    @TC006
    Scenario Outline: order higher cost product
        Given user launch the swag labs application
        When login as "VALID_UNAME" and "VALID_PWD"
        Then verify user is on product page
        Then user sort the products as "<sortAction>"
        Then user selects the first product on the inventory page
        When User clicks the Add to Cart button on the inventory item page
        Then verify the added product in the cart page
        When User clicks the cart icon on the inventory page
        Then User will be redirected to the cart page
        ...
        Examples:
            | fname | lname   | zip     | product                 | expectedMessage           | sortAction          |
            | Mark  | Anthony | 4001007 | Sauce Labs Bolt T-Shirt | Thank you for your order! | Price (low to high) |
